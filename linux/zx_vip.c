...
int disp_vip_ops(disp_info_t *disp_info, zx_capture_t *capture, void *params);
void disp_vip_notify_interrupt(disp_info_t *disp_info, zx_capture_t *capture);
void disp_vip_work_func(struct work_struct *work);

static zx_capture_desc zx_vip_descs[] =
{
    {
        .cf_fmt_id = ZX_CAPTURE_VIP1,
        .cf_name   = "VIP1"
    },
    {
        .cf_fmt_id = ZX_CAPTURE_VIP2,
        .cf_name   = "VIP2"
    },
    {
        .cf_fmt_id = ZX_CAPTURE_VIP3,
        .cf_name   = "VIP3"
    },
    {
        .cf_fmt_id = ZX_CAPTURE_VIP4,
        .cf_name   = "VIP4"
    }
};

static unsigned char cf_id2vip(zx_capture_id_t cf_fmt_id)
{
    if (cf_fmt_id == ZX_CAPTURE_VIP1)
    {
        return 0;
    }

    if (cf_fmt_id == ZX_CAPTURE_VIP2)
    {
        return 1;
    }

    if (cf_fmt_id == ZX_CAPTURE_VIP3)
    {
        return 2;
    }

    if (cf_fmt_id == ZX_CAPTURE_VIP4)
    {
        return 3;
    }
...
void disp_register_vip_capture(disp_info_t *disp_info)
{
    ...
    for (i = 0; i < sizeof(zx_vip_descs) / sizeof(zx_capture_desc); i++)
    {
        capture = (zx_capture_t *)zx_calloc(sizeof(zx_capture_t));
        capture->cf_fmt_id = zx_vip_descs[i].cf_fmt_id;
        capture->cf_name = zx_vip_descs[i].cf_name;
...
void disp_unregister_vip_capture(disp_info_t *disp_info)
{
    ...
    for (i = 0; i < sizeof(zx_vip_descs) / sizeof(zx_capture_desc); i++)
    {
        capture = disp_find_capture(disp_info, zx_vip_descs[i].cf_fmt_id);
        if (capture)
        {
...
int disp_vip_capture_init(zx_capture_t *capture, vip_op_params_t *mparams)
{
    vip_spec_info_t *info = NULL;

    capture->priv_info = zx_calloc(sizeof(vip_spec_info_t));
...
int disp_vip_capture_deinit(zx_capture_t *capture, vip_op_params_t *mparams)
{
...
int disp_vip_query_caps(disp_info_t *disp_info, zx_capture_t *capture, vip_op_params_t *mparams)
{
    vip_spec_info_t*  info = (vip_spec_info_t *)capture->priv_info;
    zx_vip_set_t v_set = {0};
    int status = 0;

    v_set.vip        = info->vip;
    v_set.op         = ZX_VIP_QUERY_CAPS;
    ...
    else
    {
        ...
        zx_info("capture %s query caps\n", capture->cf_name);
    }

    return status;
}

int disp_vip_set_mode(disp_info_t *disp_info, zx_capture_t *capture, vip_op_params_t *mparams)
{
    vip_spec_info_t*  info = (vip_spec_info_t *)capture->priv_info;
    zx_vip_set_t v_set = {0};
    int status = 0;
...
int disp_vip_set_buffer(disp_info_t *disp_info, zx_capture_t *capture, vip_op_params_t *mparams)
{
    vip_spec_info_t*  info = (vip_spec_info_t *)capture->priv_info;
    zx_vip_set_t v_set = {0};
    int status = 0;
    ...
    {
        zx_info("capture %s set buffer, idx %d, buf %x\n", capture->cf_name, mparams->set_buffer_param.fbIdx,
                mparams->set_buffer_param.fbAddr);
    }

    return status;
}

int disp_vip_capture_enable(disp_info_t *disp_info, zx_capture_t *capture, vip_op_params_t *mparams)
{
    vip_spec_info_t*  info = (vip_spec_info_t *)capture->priv_info;
    zx_vip_set_t v_set = {0};
    int status = 0;
...
int disp_vip_capture_disable(disp_info_t *disp_info, zx_capture_t *capture, vip_op_params_t *mparams)
{
    vip_spec_info_t*  info = (vip_spec_info_t *)capture->priv_info;
    zx_vip_set_t v_set = {0};
    int status = 0;
...
int disp_vip_ops(disp_info_t *disp_info, zx_capture_t *capture, void *params)
{
    ...
    case ZX_CAPTURE_OP_QUERY_CHIPINFO:
    {
        mparams->query_chipinfo_param.vipCard = ZX_VIP_CARD_AVD7611;
    }
    break;

    case ZX_CAPTURE_OP_GET_FRONT_MODE:
    {
        ...
        mparams->get_front_mode_param.mode.fmt = ZX_VIP_FMT_RGB444_24BIT_SDR;
    }
    break;

    case ZX_CAPTURE_OP_SET_PREFER_MODE:
    {
        //TODO
    }
    break;
    ...
    default:
    {
        zx_error("op not support on vip\n");
    }
    break;
    }
