...
#elif DRM_VERSION_CODE < KERNEL_VERSION(6,0,0)
...
#define ZX_FUNC_NAME(name) (((name)[0] == 'z') && ((name[1] == 'x')))?((name) + 3):(name)

#define __STR(x)    #x
#define STR(x)      __STR(x)

#define     AC_POWER_SUPPLY     1
#define     DC_POWER_SUPPLY     0

struct zx_dma_addr_t
{
    dma_addr_t dma_addr;
};
...
struct os_pages_memory
{
    ...
    struct page **pages_temp;   // it's temp workspace, avoid many zx_alloc(page_array)

    struct list_head link;  // list to g_pages_memory_list;
};
...
struct os_workqueue_struct
{
    struct workqueue_struct* wq;
};

struct os_work_struct;
