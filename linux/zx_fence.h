...
dma_fence_get_rcu_safe(dma_fence_t * __rcu *fencep)
{
    do {
        dma_fence_t *fence;
...
#define reservation_object_get_excl_rcu    dma_resv_get_excl_unlocked
...
#define reservation_object_get_excl_rcu    reservation_object_get_excl_rcu
#endif // DRM_VERSION_CODE >= KERNEL_VERSION(5,4,0)
...
typedef void (*fence_call_back)(void*, int);

struct zx_dma_fence_cb{
    dma_fence_cb_t base;
    fence_call_back call_back;
    void *adapter;
    int engine_index;
};
...
static inline int reservation_get_fences(struct reservation_object *obj, bool write, unsigned int *num_fences, dma_fence_t ***fences)
{
    ...
    if (write) {
        ...
        excl = reservation_object_get_excl_rcu(obj);
        if (excl) {
...
signed long zx_gem_object_fence_await(struct drm_zx_gem_object *obj, int exclude_self, long timeout, int write);
