...
#define list_for_each_entry_safe(pos, storage, head, member)    \
   for (pos = container_of((head)->next, typeof(*pos), member),          \
        storage = container_of(pos->member.next, typeof(*pos), member);  \
        &pos->member != (head);                                 \
        pos = storage, storage = container_of(storage->member.next, typeof(*storage), member))

#define list_for_each_entry_safe_rev(pos, storage, head, member)\
   for (pos = container_of((head)->prev, typeof(*pos), member),          \
        storage = container_of(pos->member.prev, typeof(*pos), member);  \
        &pos->member != (head);                                 \
        pos = storage, storage = container_of(storage->member.prev, typeof(*storage), member))

#define list_for_each_entry_from(pos, start, head, member)      \
   for (pos = container_of((start), typeof(*pos), member);               \
        &pos->member != (head);                                 \
        pos = container_of(pos->member.next, typeof(*pos), member))
