...
void vidschi_init_2dblt_cmd_e3k(engine_share_e3k_t *share, adapter_t *adapter)
{
    ...
    zx_memset(pKmdBitBltCmd, 0, sizeof(BITBLT_REGSETTING_E3K));
    zx_memset(pKmdFastClearCmd, 0, sizeof(FASTCLEAR_REGSETTING_E3K));
    ...
    {
        pKmdBitBltCmd->Set_Ff_Glb_Ctrl = SET_REGISTER_FD_E3K(FF_BLOCK, Reg_Ff_Glb_Ctrl_Offset, 1);
        pKmdBitBltCmd->Ff_Glb_Ctrl.reg.Ffc_Config_Mode = FF_GLB_CTRL_FFC_CONFIG_MODE_D_ONLY;
    }
    ...
    if (adapter->chip_id >= CHIP_CHX004)
    {
        pKmdFastClearCmd->Set_Ff_Glb_Ctrl = SET_REGISTER_FD_E3K(FF_BLOCK, Reg_Ff_Glb_Ctrl_Offset, 1);
        pKmdFastClearCmd->Ff_Glb_Ctrl.reg.Ffc_Config_Mode = FF_GLB_CTRL_FFC_CONFIG_MODE_D_ONLY;
    }
...
void vidschi_init_blt_cmd_e3k(engine_share_e3k_t *share, adapter_t *adapter)
{
    vidschi_init_2dblt_cmd_e3k(share, adapter);
}
